...
// pit stops in it. "compare": multi-scenario mode — up to MAX_SCENARIOS
// independent single-pit-lap alternatives ("lap 30 vs 33 vs 36"), run
// against the identical field state and compared side by side. Mirrors
// web/src/pages/SimulatorPage.tsx's SimulationMode — deliberately separate
// mental models, not merged into one UI.
...
// PitStopRow's sequential multi-stop plan). Mirrors web's ScenarioRow.
...
// available, joined by " · ", or "" (falsy, so the caller can skip rendering
  ...
  return (
    <div className="space-y-3 rounded-lg border bg-muted/30 p-4">
        ...
        <div className="space-y-1">
          ...
          <div className="space-y-1">
            {explanation.drivers_overtaken.map((entry) => {
              ...
              return (
                <div key={entry.driver_id} className="py-0.5">
                  ...
                  {enrichmentText && (
                    <p className="pl-9 font-mono text-[10px] text-muted-foreground/80">
                      {enrichmentText}
                    </p>
                  )}
                </div>
      ...
      {explanation.remaining_laps > 0 && freshCompound && (
        <p className="text-xs text-muted-foreground">
          {degradationMessage} This is a simplified snapshot that assumes rivals hold their current pace
          with no further pit stops of their own — the Monte Carlo position change above already accounts
          for rivals' own tyre wear and pit stops, so treat that as the number to trust and this line as
          partial context, not the full picture.
        </p>
      )}
    </div>
  )
...
export function SimulatorPage() {
  ...
  // Driver defaults from the race context (already selected in the app
  // shell for tray/notifications) but stays freely editable — once picked
  // (from context or manually), default Current Lap/Compound/Tyre Age from
  // their latest lap in this session. lastAutoFilledDriverRef guards
  // against clobbering a manual edit: it only applies once per driver
  // selection, not on useDriverLaps' background 10s poll (which changes
  // .data every tick without driverId changing).
  ...
  async function handleRunSimulation() {
    ...
    // — compare mode sends only scenarios, single-plan mode sends only
    // pit_laps/compounds. Mirrors web/src/pages/SimulatorPage.tsx.
  ...
  async function handleExportCsv() {
    setIsExporting(true)
    try {
      const saved = await exportStrategiesAsCsv(strategies, startingPosition)
      if (saved) toast.success("Results exported")
    } catch (error) {
  ...
  const chartData = strategies.map((strategy, index) => ({
    ...
    // fallback naming as before for the single-plan path.
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {/* Single Plan (sequential multi-stop) vs Compare Scenarios
                (independent single-pit-lap alternatives). Mirrors
                web/src/pages/SimulatorPage.tsx's SimulationMode toggle. */}
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex gap-2">
              ...
              {/* Desktop-exclusive — no equivalent in web/src/pages/SimulatorPage.tsx. */}
              <Button
                variant="outline"
                disabled={strategies.length === 0 || isExporting}
