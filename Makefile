.PHONY: install dev dev-down test test-unit test-int test-integration test-e2e lint type-check migrate new-migration train seed seed-circuits seed-teams ingest ingest-season backfill-tire-data backfill-lap-session-time backfill-session-total-laps ingest-live warm-cache fly-race-up fly-race-down
...
backfill-session-total-laps:
	python backend/scripts/backfill_session_total_laps.py $(if $(SEASON),--season $(SEASON),) $(if $(ROUND),--round $(ROUND),)
