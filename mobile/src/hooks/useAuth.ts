...
// Hand-written, not copied — mirrors web/src/hooks/useAuth.ts's logic
// exactly (pure react-query/zustand, no browser APIs involved). See
...
export function useAuth() {
  ...
  // Self-heals the case where a token exists (rehydrated from SecureStore)
  // but the persisted user is missing — expected on every mobile launch,
  // since authStore never persists `user` at all (see stores/authStore.ts).
